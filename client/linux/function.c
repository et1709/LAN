	...
	while(1)
	{
		if(receive_queue->front != receive_queue->rear)
		{
			...
			switch(receive_data.order)
			{
				...
				case 22:
					printf("   登陆成功!\n");
					strcpy(myNickname, receive_data.nickname);
		...
		{					
			...
			{			
				...
				else
				{
					...
					{
						switch(send_data.order)
						{
							case 3:
								printf("%s(%s)(%s):", send_data.nickname, 
										send_data.information, send_data.send_msg_time);
								break;
							case 4:     //查看好友信息
								printf("   发送请求 order:%d 成功, information: %s\n",
...
int single_chat(void)
{
	...
	find_friend_list();
	usleep(300000);
	strcpy(data.nickname, myNickname);
	while(1)
	{
